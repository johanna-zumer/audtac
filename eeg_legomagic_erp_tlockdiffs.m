...
for ii=iiuse
% for ii=setdiff(iiuse,[8:14])
% for ii=[28 9 16 18 24]
% for ii=28
  close all
  cd([edir sub{ii} ])
  clearvars -except ii sub edir ddir ii*use sleep featurestats* ttuse soades usetr tacaloneproc synchasynch phaset0* binonly pcmflag plothist iter* trialkc* subuseall trl* stim2nd
            ...
            if sleep && ss==12
              ...
              elseif trlfeat==2
                ...
                if min_nKD(rem(ll,10))>20
                  ...
                  cfg.trials=Shuffle(find(featind_tac{ll}.KcDuring));
                  tlock_tactlock_KD{ll,tt,ss}=ft_timelockanalysis(cfg,tlock_tac{ll,tt,ss});
%                   figure(ll);plot(tlock_tactlock_KD{ll,tt,ss}.time,[tlock_tactlock_KD{ll,tt,ss}.avg(17,:)' tlock_tactlock_nKD{ll,tt,ss}.avg(17,:)']);ylim([-60 20])
%                   print(ll,[pwd '/tlock_tacll' num2str(ll) '_KDnKD.eps'],'-depsc')
                else
                ...
                cfg.lpfreq=15;
                cfg.channel='Fz';
                tmp1=ft_preprocessing(cfg,tlock_tac{ll,tt,ss});
                cfg=[];
                cfg.resamplefs=40;
                cfg.time=repmat({[.4:.025:.75]+stim2nd(rem(ll,10))},[1 size(tmp1.trial,1)]);
                tmp2=ft_resampledata(cfg, tmp1);
                cfg=[];
                cfg.keeptrials='yes';
%                 cfg.latency=[.4 .75]+soades(rem(ll,10));
%                 tlock_tacKcaud{ll}=ft_timelockanalysis(cfg,tmp2);                
%                 cfg.latency=[.4 .75]+stim2nd(rem(ll,10));
                tlock_tacKc2nd{ll}=ft_timelockanalysis(cfg,tmp2);
          ...
          if phaset0 && phaset0use==0
            ...
            if sleep && ss==12
              if trlfeat==1
                ...
                featind_aud{ll} = feat_pre_during_evoked_individual(tlock_aud{ll,tt,ss},ll); %
              elseif trlfeat==2
                tcfg=cfg;cfg=[];cfg.randomseed=13;ft_preamble randomseed;cfg=tcfg;  % Sets call to 'rand' in Shuffle to same starting point, for reproducibility
                if min_nKD(rem(ll,10))>20
                  ...
                  cfg.trials=Shuffle(find(featind_aud{ll}.KcDuring));
                  tlock_audtlock_KD{ll,tt,ss}=ft_timelockanalysis(cfg,tlock_aud{ll,tt,ss});
%                   figure(ll);plot(tlock_audtlock_KD{ll,tt,ss}.time,[tlock_audtlock_KD{ll,tt,ss}.avg(17,:)' tlock_audtlock_nKD{ll,tt,ss}.avg(17,:)']);ylim([-60 20])
%                   print(ll,[pwd '/tlock_audll' num2str(ll) '_KDnKD.eps'],'-depsc')
                else
                ...
                cfg.lpfreq=15;
                cfg.channel='Fz';
                tmp1=ft_preprocessing(cfg,tlock_aud{ll,tt,ss});
                cfg=[];
                cfg.resamplefs=40;
                cfg.time=repmat({[.4:.025:.75]+stim2nd(rem(ll,10))},[1 size(tmp1.trial,1)]);
                tmp2=ft_resampledata(cfg, tmp1);
                ...
                cfg.keeptrials='yes';
%                 cfg.latency=[.4 .75]+soades(rem(ll,10));
%                 tlock_audKcaud{ll}=ft_timelockanalysis(cfg,tmp2);                
%                 cfg.latency=[.4 .75]+stim2nd(rem(ll,10));
                tlock_audKc2nd{ll}=ft_timelockanalysis(cfg,tmp2);
              end
            end
            if pcmflag
              if rem(ll,10)==1 || rem(ll,10)==3 || rem(ll,10)==4 || rem(ll,10)==5
        ...
        if phaset0 && phaset0use==0
          ...
          if sleep && ss==12
            if trlfeat==1
              ...
              featind_nul{ll} = feat_pre_during_evoked_individual(tlock_nul{ll,tt,ss},ll); %
            elseif trlfeat==2
              tcfg=cfg;cfg=[];cfg.randomseed=13;ft_preamble randomseed;cfg=tcfg;  % Sets call to 'rand' in Shuffle to same starting point, for reproducibility
              if min_nKD(rem(ll,10))>20
                ...
                tlock_nultlock_nKD{ll,tt,ss}=rmfield(tlock_nultlock_nKD{ll,tt,ss},{'var' 'dof'});
                  cfg.trials=Shuffle(find(featind_nul{ll}.KcDuring));
                  tlock_nultlock_KD{ll,tt,ss}=ft_timelockanalysis(cfg,tlock_nul{ll,tt,ss});
%                   figure(ll);plot(tlock_nultlock_KD{ll,tt,ss}.time,[tlock_nultlock_KD{ll,tt,ss}.avg(17,:)' tlock_nultlock_nKD{ll,tt,ss}.avg(17,:)']);ylim([-60 20])
%                   print(ll,[pwd '/tlock_nulll' num2str(ll) '_KDnKD.eps'],'-depsc')
              else
              ...
              cfg.lpfreq=15;
              cfg.channel='Fz';
              tmp1=ft_preprocessing(cfg,tlock_nul{ll,tt,ss});
              cfg=[];
              cfg.resamplefs=40;
              cfg.time=repmat({[.4:.025:.75]+stim2nd(rem(ll,10))},[1 size(tmp1.trial,1)]);
              tmp2=ft_resampledata(cfg, tmp1);
              ...
              cfg.keeptrials='yes';
%               cfg.latency=[.4 .75]+soades(rem(ll,10));
%               tlock_nulKcaud{ll}=ft_timelockanalysis(cfg,tmp2);
%               cfg.latency=[.4 .75]+stim2nd(rem(ll,10));
              tlock_nulKc2nd{ll}=ft_timelockanalysis(cfg,tmp2);
            end
      ...
      if numt_trials(ll,tt,ss)>=2
        ...
        if sleep==1 && ss==12 && trlfeat==2
          ...
          try
            ...
            tlock_tTacAlone_nKD{ll,tt,ss}=tlock_tactlock_nKD{ll+20,tt,ss}; % keeping tac centred but jitter aud (to match tlock_tac{X,tt})
            tlock_tAudAlone_nKD{ll,tt,ss}=tlock_audtlock_nKD{ll+40,tt,ss}; % keeping tac centred but jitter aud (to match tlock_tac{X,tt})
          catch
            tlock_tacPaud_nKD{ll,tt,ss}=[];
            tlock_tTacAlone_nKD{ll,tt,ss}=[];
            tlock_tAudAlone_nKD{ll,tt,ss}=[];
          end
          try
            ...
            tlock_tTacAlone_nSD{ll,tt,ss}=tlock_tactlock_nSD{ll+20,tt,ss}; % keeping tac centred but jitter aud (to match tlock_tac{X,tt})
            tlock_tAudAlone_nSD{ll,tt,ss}=tlock_audtlock_nSD{ll+40,tt,ss}; % keeping tac centred but jitter aud (to match tlock_tac{X,tt})
          catch
            tlock_tacPaud_nSD{ll,tt,ss}=[];
            tlock_tTacAlone_nSD{ll,tt,ss}=[];
            tlock_tAudAlone_nSD{ll,tt,ss}=[];
          end
          try
            ...
            tlock_tTacAlone_nKD_nSD{ll,tt,ss}=tlock_tactlock_nKD_nSD{ll+20,tt,ss}; % keeping tac centred but jitter aud (to match tlock_tac{X,tt})
            tlock_tAudAlone_nKD_nSD{ll,tt,ss}=tlock_audtlock_nKD_nSD{ll+40,tt,ss}; % keeping tac centred but jitter aud (to match tlock_tac{X,tt})
          catch
            tlock_tacPaud_nKD_nSD{ll,tt,ss}=[];
            tlock_tTacAlone_nKD_nSD{ll,tt,ss}=[];
            tlock_tAudAlone_nKD_nSD{ll,tt,ss}=[];
          end
        end
        if phaset0
          cfg.parameter='avg';
      ...
      if numt_trials(ll,tt,ss)>=2
        ...
        if sleep==1 && ss==12 && trlfeat==2
          ...
          try
            ...
            tlock_tMSAlone_nKD{ll,tt,ss}=tlock_tactlock_nKD{ll,tt,ss}; % TA+N
            tlock_tNulAlone_nKD{ll,tt,ss}=tlock_nultlock_nKD{ll+50,tt,ss}; % TA+N
          catch
            tlock_tacMSpN_nKD{ll,tt,ss}=[];
            tlock_tMSAlone_nKD{ll,tt,ss}=[];
            tlock_tNulAlone_nKD{ll,tt,ss}=[];
          end
          try
            tlock_tacMSpN_nSD{ll,tt,ss}=ft_math(cfg,tlock_tactlock_nSD{ll,tt,ss},tlock_nultlock_nSD{ll+50,tt,ss}); % TA+N
            tlock_tMSAlone_nSD{ll,tt,ss}=tlock_tactlock_nSD{ll,tt,ss}; % TA+N
            tlock_tNulAlone_nSD{ll,tt,ss}=tlock_nultlock_nSD{ll+50,tt,ss}; % TA+N
          catch
            tlock_tacMSpN_nSD{ll,tt,ss}=[];
            tlock_tMSAlone_nSD{ll,tt,ss}=[];
            tlock_tNulAlone_nSD{ll,tt,ss}=[];
          end
          try
            tlock_tacMSpN_nKD_nSD{ll,tt,ss}=ft_math(cfg,tlock_tactlock_nKD_nSD{ll,tt,ss},tlock_nultlock_nKD_nSD{ll+50,tt,ss}); % TA+N
            tlock_tMSAlone_nKD_nSD{ll,tt,ss}=tlock_tactlock_nKD_nSD{ll,tt,ss}; % TA+N
            tlock_tNulAlone_nKD_nSD{ll,tt,ss}=tlock_nultlock_nKD_nSD{ll+50,tt,ss}; % TA+N
          catch
            tlock_tacMSpN_nKD_nSD{ll,tt,ss}=[];
            tlock_tMSAlone_nKD_nSD{ll,tt,ss}=[];
            tlock_tNulAlone_nKD_nSD{ll,tt,ss}=[];
          end
        end
        if phaset0
          cfg.parameter='avg';
  ...
  if ~tacaloneproc && ~phaset0 && ~pcmflag && trlfeat==2
    if isempty(tr.stageuse)
      ...
      save(['tlock_diffs_averef_' sub{ii} '_sleep' num2str(sleep) '_tt' num2str(tt) '_tacaud' num2str(tacaud) '_iter' num2str(iter) '_trialkc' num2str(trialkc) '.mat'],'tlock_*P*','tlock_*N*','tlock_t*','tlock_a*','tlock_n*','tlock*tlock*','num*trials','featurestats_*')
    end
  elseif phaset0 && phaset0use>0
    if binonly
    else
