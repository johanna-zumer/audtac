function [trl, event, trlselexcl] = ft_trialfun_general_sleep(cfg)
...
%
% $Id: ft_trialfun_general.m 9586 2014-05-23 11:10:03Z roboos $
...
%% Added for sleep scoring 
% size of sleep-score chunk in samples
chunksamp=hdr.orig.other.CRC.score{3}*hdr.Fs;
% create sample-by-sample vector of sleep score
for ll=1:length(hdr.orig.other.CRC.score{1})
    samplesleep((ll-1)*chunksamp+1:(ll*chunksamp),1)=hdr.orig.other.CRC.score{1}(ll);
end

% add as new field to events structure the sleep score
% for ll=1:length(hdr.orig.trials.events)
%     hdr.orig.trials.events(ll).sleep= samplesleep(dsearchn(cfg.time{1}',hdr.orig.trials.events(ll).time));
% end
for ll=1:length(event)
    event(ll).sleep= samplesleep(event(ll).sample);
end

try
  save([cfg.dataset(1:end-4) '_event.mat'],'event')
catch % sometimes unable to write file (permission denied)
  pause(10)
  save([cfg.dataset(1:end-4) '_event.mat'],'event')
end  
% end of added code
%% return to normal code from general trialfun
...
for i=sel
  ...
  if trlbeg>0 && trlend<=hdr.nSamples.*hdr.nTrials,
    ...
    elseif ischar(event(i).value) && (event(i).value(1)=='S'|| event(i).value(1)=='R')
      ...
      if strcmp(cfg.trialdef.eventvalue,'S 20') % S 20 is at start of trial, so we want the value just after this one
        ind=-1;
      else % go backwards in events within the trial until find the condition index
        ind=1;
        try
          while str2num(event(i-ind).value(2:end))>50 | str2num(event(i-ind).value(2:end))<35
            ind=ind+1;
          end
        catch
          ind=1;
      ...
      if strcmp(cfg.trialdef.eventvalue,'S 20') & str2double(event(i-1).value(2:end))==7 % hack, to know when start of next block occurs
        val = [val; [-7 event(i).sleep str2double(event(i-ind).value(2:end))-40]  ];
      else
        val = [val; [str2double(event(i).value(2:end)) event(i).sleep str2double(event(i-ind).value(2:end))-40]  ];
      end
%       val = [val; [str2double(event(i).value(2:end)) event(i).sleep] ];
    else
      val = [val; nan];
    end
  else
    trlselexcl=[trlselexcl find(sel==i)]; % keep track of trials excluded b/c they were outside the data boundaries.
  end
end
