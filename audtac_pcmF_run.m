...
% load([sdir 'sim7NR_featmodels.mat']);  % from create_sim5_models.m
catch
...
[A.Tgroup,A.theta,A.Gpred] = pcm_fitModelGroup(Y,M,partVecch,Xchunk,'runEffect','fixed','fitScale',1);
save([edir 'pcmF_datafit_TS17_Msim4bNR_nc18np2_jj' num2str(jj) '.mat'],'A','partVecch','M'); % using real partVec
[A.Tcross,A.thetaCr,A.G_predcv] = pcm_fitModelGroupCrossval(Y,M,partVecch,Xchunk,'runEffect','fixed','groupFit',A.theta,'fitScale',1);
save([edir 'pcmF_datafit_TS17_Msim4bNR_nc18np2_jj' num2str(jj) '.mat'],'A','partVecch','M'); % using real partVec
