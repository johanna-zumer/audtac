...
for i=sel
  ...
  if ~isfield(cfg.trialdef, 'prestim')
    ...
    error('no prestim specified');
  else
      tmpind=1;
      while ~strcmp(event(i-tmpind).value,cfg.trialdef.prestim)
          tmpind=tmpind-1;
      end
      trlbeg = event(i-tmpind).sample;
      trloff = trlbeg - event(i).sample;
...
%     trlbeg = event(i).sample + trloff;
  ...
  if ~isfield(cfg.trialdef, 'poststim')
    ...
    error('no poststim specified');
  else
      tmpind=1;
      val1=[];
      while isempty(match_str(event(i+tmpind).value,cfg.trialdef.poststim))
          val1=[val1 str2num(event(i+tmpind).value(end-1:end)) event(i+tmpind).sample-event(i).sample];
          tmpind=tmpind+1;          
      end
      trlend = event(i+tmpind).sample;
      trldur = trlend - trlbeg;
